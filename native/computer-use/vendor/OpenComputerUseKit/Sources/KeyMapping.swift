...
extension KeyPressParser {
    /// Text a key code produces under the given modifiers, or `""` for keys that
    /// emit nothing typeable (arrows, function keys, Escape…).
    ///
    /// The official builder sets a unicode string on *every* key event
    ///, so a plain `pressKey("a")` still carries "a". Named keys
    /// legitimately have no text; sending an empty string for those matches the
    /// old behaviour and is what the receiver expects.
    static func characters(for keyCode: CGKeyCode, modifiers: CGEventFlags) -> String {
        KeyboardLayoutMap.characters(keyCode: keyCode, modifiers: modifiers)
    }
}
