...
enum SoftwareCursorGlyphRenderer {
    ...
    /// The contour table below was traced off the official cursor bitmap, so its
    /// rows are in **image coordinates — y grows downward**. `rect` lives in an
    /// AppKit/CoreGraphics context where y grows *upward*, so the mapping has to
    /// invert y. Without the inversion the arrow renders tip-down.
    static func pointerPath(in rect: CGRect) -> NSBezierPath {
        ...
        func mappedPoint(x: CGFloat, y: CGFloat) -> CGPoint {
            CGPoint(
                ...
                y: rect.maxY - ((y - sourceMinY) / (sourceMaxY - sourceMinY) * rect.height)
            )
        }
...
func loadReferenceCursorWindowImage() -> NSImage? {
    ...
    // …/native/computer-use/vendor/OpenComputerUseKit/Sources/<this file>
    // needs six pops to reach the repo root; five stopped at `native/`, so this
    // lookup could never succeed.
    //
    // Note the asset itself is **not in the repo** and no build step bundles it,
    // so `referenceImage` is nil everywhere and the procedural glyph above is
    // what actually ships. Keep this path correct in case the asset is added.
