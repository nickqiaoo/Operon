...
import type { CommandExecution, FileChange, McpToolCall, WebSearch, CollabAgentToolCall } from '../protocol/index.js';
...
export function resolveToolName(item: ToolItem): { toolName: string; dynamic?: boolean } {
  ...
  if (isCollabAgentToolCall(item)) {
    return { toolName: 'codex_collab_agent', dynamic: true };
  }
